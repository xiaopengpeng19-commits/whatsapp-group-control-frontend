        ...
        <div style="margin-top:20px;">
          ...
          <el-table :data="groupDetail.participants || []" border size="small" max-height="300">
            ...
            <el-table-column label="操作" width="180" align="center">
              <template #default="{ row }">
                <div style="display:flex;gap:4px;flex-wrap:wrap;">
                  <!-- 不能操作自己（用 phoneNumber 和 selectedAccount 比对） -->
                  <template v-if="getPurePhone(row.phoneNumber) !== selectedAccount">
                    <!-- 提拔（仅对普通成员） -->
                    <el-button
                      v-if="row.admin !== 'superadmin' && row.admin !== 'admin'"
                      size="small"
                      type="warning"
                      plain

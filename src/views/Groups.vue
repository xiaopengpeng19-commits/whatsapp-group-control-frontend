    ...
    <div class="groups">
        ...
        <div class="toolbar">
            <div style="display:flex;gap:10px;align-items:center;flex-wrap:wrap;">
                ...
                <!-- 分组筛选 -->
                <el-select v-model="filterGroup" placeholder="全部分组" clearable @change="fetchAccounts"
                    style="width:150px">
            ...
            <el-table :data="groupList" border v-loading="groupLoading" max-height="450">
                ...
                <el-table-column label="操作" width="180" align="center" fixed="right">
                    <template #default="{ row }">
                        ...
                        <el-button v-if="row.status === 'active'" size="small" type="success" plain
        ...
        <!-- 群组详情弹窗 -->
        <!-- ========================================== -->
        <el-dialog v-model="showGroupDetailDialog"
            :title="`群组详情 - ${groupDetail?.subject || groupDetail?.groupId || ''}`" width="750px"
                ...
                <div style="margin-top:20px;">
                    ...
                    <el-table :data="groupDetail.participants || []" border size="small" max-height="300">
                        ...
                        <el-table-column label="角色" width="100" align="center">
                            <template #default="{ row }">
                                <el-tag
                                    :type="row.admin === 'superadmin' ? 'danger' : row.admin === 'admin' ? 'warning' : 'info'"
                                    size="small">
                        ...
                        <el-table-column label="操作" width="180" align="center">
                            <template #default="{ row }">
                                <div style="display:flex;gap:4px;flex-wrap:wrap;">
                                    <!-- 不能操作自己（用 phoneNumber 比对） -->
                                    <template v-if="getPurePhone(row.phoneNumber) !== currentAccount">
                                        <!-- 提拔（仅对普通成员） -->
                                        <el-button v-if="row.admin !== 'superadmin' && row.admin !== 'admin'"
                                            size="small" type="warning" plain @click="handlePromote(row)">
                                            提拔
                                        </el-button>
                                        <!-- 降级（仅对管理员） -->
                                        ...
                                        <!-- 移除（不能移除群主和自己） -->
                                        <el-button v-if="row.admin !== 'superadmin'" size="small" type="danger" plain
                                            @click="handleRemove(row)">
                                            移除
                                        </el-button>
                                    </template>
                                    <span v-else style="color:#999;font-size:12px;">自己</span>
                                </div>
                            </template>
                        </el-table-column>
                    </el-table>
...
const getCurrentAccount = () => {
    ...
    if (userInfo && userInfo.account) {
        currentAccount.value = userInfo.account
    }
    // 如果还没有，尝试从账号列表中取第一个作为默认
    if (!currentAccount.value && accounts.value.length > 0) {
        currentAccount.value = accounts.value[0]?.account || ''
    }
}
